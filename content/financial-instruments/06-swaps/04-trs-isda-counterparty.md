...
Что мы теряем, если контрагент разорится? Ответ несимметричен. Если своп был для нас в минусе, мы ничего не теряем: обязательство никуда не делось, заплатим его конкурсной массе. А вот если своп был в плюсе, мы теряем всю эту прибыль — заменить сделку придётся уже по новым рыночным ценам.

Значит, риск определяется только **положительной** частью стоимости. Её ожидаемое значение на дату $t$ называется **ожидаемой экспозицией**: $EE(t) = \mathbb{E}[\max(V_t, 0)]$, где $V_t$ — стоимость сделки в момент $t$, а $\max(V_t, 0)$ отсекает случаи, когда мы в минусе.

Теперь соберём цену этого риска. Для каждого будущего периода перемножим три числа: сколько мы потеряем, если дефолт случится именно тогда; вероятность, что он случится именно в этом периоде; и дисконт-фактор, чтобы привести к сегодняшнему дню. Сумма даёт **корректировку на кредитный риск контрагента** (credit valuation adjustment, CVA):
...
где:
- $1 - R$ — доля потерь при дефолте: $R$ — доля возмещения из модуля 4;
- $EE(t_i)$ — ожидаемая экспозиция на дату $t_i$;
- $Q(t) = e^{-\lambda t}$ — вероятность дожить до $t$ без дефолта, а разность $Q(t_{i-1}) - Q(t_i)$ — вероятность дефолта **внутри** периода $i$;
- $P(0,t_i)$ — дисконт-фактор.

CVA вычитается из безрисковой стоимости сделки: именно так банк учитывает, что клиент может не дожить до конца свопа.
...
Саму экспозицию $EE(t)$ аналитически не получить: нужно знать распределение будущих стоимостей свопа. Её считают методом Монте-Карло — прогоняют тысячи сценариев рыночных ставок и курсов и усредняют положительные части. Этот метод появится в модуле 10, а полный набор корректировок (XVA) — тема для дальнейшего изучения (модуль 11).
