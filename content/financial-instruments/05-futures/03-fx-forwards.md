...
Котировка валютной пары — цена одной единицы базовой валюты в котируемой. В записи CNY/RUB = 11,00 юань — **базовая** валюта (её покупают), рубль — **котируемая** (в ней измеряют цену): 11 рублей за юань.

Обозначения:
- $S$ — спот-курс, то есть курс для расчётов сегодня;
- $F$ — форвардный курс, зафиксированный сегодня для обмена через срок $\tau$;
- $r_d$ — ставка в котируемой валюте (domestic — «внутренняя», у нас рубль);
- $r_f$ — ставка в базовой валюте (foreign — «иностранная», у нас юань).

Теперь главная мысль. У владельца одного юаня есть два способа получить рубли через год, и оба безрисковые:
1. положить юань на депозит под $r_f$, а получившиеся юани обменять по форвардному курсу $F$, о котором договорились уже сегодня;
2. обменять юань на рубли прямо сейчас по курсу $S$ и положить рубли под $r_d$.

Если бы один путь давал больше рублей, все выбрали бы его, а разницу забирали бы арбитражёры. Значит, результаты совпадают — это и есть **покрытый процентный паритет** (covered interest parity, CIP):

$$
F = S\cdot\frac{1 + r_d\,\tau_d}{1 + r_f\,\tau_f} \qquad \text{или в непрерывной форме} \qquad F = S\,e^{(r_d - r_f)\tau}.
$$

Здесь $\tau_d$ и $\tau_f$ — доли года, посчитанные по конвенциям своих валют (модуль 2): у рубля делитель 365, у юаня 360. Слово «покрытый» означает, что курс обмена зафиксирован заранее, то есть валютный риск закрыт.

Формула — частный случай стоимости владения из предыдущего урока: иностранная валюта ведёт себя как актив, который сам приносит доход по ставке $r_f$, вроде акции с дивидендами.
