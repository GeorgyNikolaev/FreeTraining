...
**Овернайт** (overnight) — заём на одну ночь, до следующего рабочего дня. Банки занимают друг у друга именно так: у одного к вечеру деньги в избытке, у другого не хватает на обязательные резервы.

Центральный банк управляет стоимостью таких денег. Банк России:
- выдаёт банкам кредиты под **ключевую ставку плюс 1 п.п.**;
- принимает депозиты под **ключевую ставку минус 1 п.п.**;
- на аукционах даёт банкам деньги и забирает лишние по ставкам около ключевой.

Ни один банк не займёт у другого дороже, чем у ЦБ (проще прийти к ЦБ), и не разместит дешевле, чем в ЦБ. Поэтому межбанковские ставки на один день заперты в **процентном коридоре** шириной 2 п.п. вокруг ключевой ставки. Так решение ЦБ о ставке доходит до всего рынка.
...
Ставка овернайт живёт один день, а купон по флоатеру нужно платить раз в квартал. Как из 63 дневных ставок получить одну квартальную? Представим, что деньги каждый день переразмещаются под ставку этого дня, и посчитаем, во что они превратились за квартал. Это и называется **капитализацией ставки за период** (compounding in arrears):
...
где:
- $\prod$ — произведение по всем дням периода: множители перемножаются, потому что проценты каждого дня начисляются на уже подросшую сумму;
- $r_i$ — ставка овернайт в день $i$;
- $d_i$ — на сколько календарных дней она действует: в будни 1, в пятницу 3 (выходные);
- $n$ — число значений ставки в периоде;
- $D = \sum d_i$ — длина всего периода в календарных днях;
- $B$ — база года из конвенции подсчёта дней (модуль 2): 365 для рубля, 360 для доллара;
- множитель $B/D$ переводит доход за период обратно в годовую ставку.
