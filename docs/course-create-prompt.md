...
1. **Сначала план.** Название, описание, уровень, перечень модулей с одной
   строкой о каждом и список уроков внутри каждого модуля. Покажи план и дождись правок.
...
prerequisites:             # необязателен, но почти всегда уместен
  - Читать SQL-запросы с JOIN и группировкой
  - Понимать, чем HTTP-запрос отличается от вызова функции
```
